...
const selectVariants = cva(
  "w-full px-3 h-10 text-base flex items-center justify-between rounded-md border border-input transition duration-300 read-only:bg-background disabled:cursor-not-allowed disabled:opacity-50 [&>svg]:h-5 [&>svg]:w-5 [&>span]:text-base [&>span]:font-normal",
  {
    variants: {
      color: {
        default:
          "border-default-200 text-default-500 outline-none focus:outline-none focus:ring-0 focus:border-primary data-[state=open]:border-primary disabled:bg-default-200 placeholder:text-accent-foreground/50 [&>svg]:stroke-default-600 cursor-pointer",
        primary:
      ...
      size: {
        sm: "h-8 text-sm [&>span]:text-sm",
        default: "h-9 text-base [&>span]:text-base",
        md: "h-10 text-base [&>span]:text-base",
        lg: "h-12 text-lg [&>span]:text-lg",
      },
    },
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex w-full cursor-pointer select-none items-center rounded-sm py-1.5 ps-4 pe-2 text-base outline-none transition-colors duration-200 focus:bg-primary focus:text-primary-foreground data-[highlighted]:bg-primary data-[highlighted]:text-primary-foreground data-disabled:pointer-events-none data-disabled:opacity-50",
      className,
    )}
    {...props}
  >
